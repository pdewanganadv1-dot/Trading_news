...
def _build_help() -> str:
    return (
        ...
        "📡 *Live Market (Dhan WebSocket)*\n"
        "• `/live <sym>` — Live price, volume, OHLC\n"
        "• `/gainers` — Top 10 gainers by %\n"
        "• `/losers` — Top 10 losers by %\n"
        "• `/breadth` — Stocks above/below day open\n"
        "• `/edges` — Top 15 live edge scores\n"
        "• `/feed` — WebSocket feed health status\n\n"
        "🔥 *Edge Scanner (yfinance)*\n"
        "• `edges` — Top 10 stocks by edge score (0-10)\n"
        "• `edge <symbol>` — Edge scan for any stock\n"
        "• `breadth` — % of Nifty 100 above 20-day SMA\n"
        "• `fiidii` — FII/DII institutional flow + 5-day trend\n"
        "• `setfiidii <FII_b> <FII_s> <DII_b> <DII_s>` — Update FII/DII\n\n"
        "⚡ *SCALP Scanner*\n"
        "• `/scalp` — EMA 200 bounce signals (1min, 119 stocks)\n"
        "• `/scalpbt` — Backtest EMA 200 strategy (6mo daily)\n"
        "• `/scalpon` — Enable auto-scan\n"
        "• `/scalpoff` — Disable auto-scan\n\n"
        "🏦 *DhanHQ Broker*\n"
        "• `/dhan` — Dashboard (funds, profile, positions)\n"
        "• `/buy <sym> <qty>` — Place BUY order\n"
        "• `/sell <sym> <qty>` — Place SELL order\n"
        "• `/dhanon` — Enable auto-trading\n"
        "• `/dhanoff` — Disable auto-trading\n\n"
        "🧠 *AI Agent*\n"
        "• `/agent <symbol>` — Groq multi-modal analysis (price+news+social+FII/DII)\n"
        "• `/social <symbol>` — StockTwits + Reddit sentiment\n\n"
        "📊 *Options & Derivatives*\n"
        "• `/options <symbol>` — Option chain, PCR, max pain, key levels\n\n"
        "🏛 *Insider & Political Trades*\n"
        "• `/insider` — Bulk & block deals summary (1M)\n"
        ...
        "📰 *News & Sentiment*\n"
        "• `sentiment` — Nifty market sentiment overview\n"
        "• `sentiment <symbol>` — Cached news sentiment\n"
        "• `<symbol>-news` — Fetch live news for any symbol\n\n"
        "💹 *Price & Charts*\n"
        "• `<symbol>` — Price + signal + chart (119 Nifty stocks, BTC, ETH, Gold, Silver)\n"
        "• `/chart <symbol>` — Candlestick chart with EMA9/20\n"
        "• `alert <sym> above|below <price>` — Set price alert\n"
        "• `alerts` — List active alerts\n"
        "• `remove alert <id>` — Remove alert\n\n"
        "📈 *Accuracy*\n"
        "• `/accuracy` — Signal win/loss/PnL stats\n\n"
        "🌅 *Daily Reports*\n"
        "• `premarket` — 9am: global cues, FII/DII, outlook\n"
        ...
        "• `/markets` — All features overview\n\n"
        "💡 Commands are case-insensitive. Type any symbol for instant price + signal."
    )
